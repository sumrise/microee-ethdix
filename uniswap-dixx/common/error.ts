...
export class ErrorHandler extends Error {
  ...
  static handle(errCode: number, err: Error, res: express.Response) {
    if (err instanceof AssertionError) {
      const code = 413;
      const { message } = err;
      res.status(200).json({ code: code, message: message.split(':')[0] });
      return;
    }
    ...
    loggerError(`type=${typeof err}, errorCode=${errCode}, errorMessage=${err.message}, stack=${err.stack}`);
    res.status(errCode).json({ code: errCode, message: err.message });
  }
}
