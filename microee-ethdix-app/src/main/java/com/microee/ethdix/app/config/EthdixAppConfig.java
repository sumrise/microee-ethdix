...
public class EthdixAppConfig implements ApplicationListener<ApplicationEvent> {
    ...
    public JsonRPC jsonRPCClientMainnet() {
        ...
        try {
            return new JsonRPC(ethNetworkProperties.getMainnetNodes(), ChainId.MAINNET, ethNetworkProperties.getMainnetWss(), ethBlockListener).connect();  
        } catch (Exception e) {
    ...
    public JsonRPC jsonRPCClientRopsten() {
        ...
        try {
            return new JsonRPC(ethNetworkProperties.getRopstenNodes(), ChainId.ROPSTEN, ethNetworkProperties.getRopstenWss(), ethBlockListener).connect();  
        } catch (Exception e) {
            LOGGER.error("errorMessage={}", e.getMessage(), e);
        }
        return null;
